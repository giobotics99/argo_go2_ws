  ...
  go2_interfaces__srv__FootRaiseHeight_Request__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    go2_interfaces__srv__FootRaiseHeight_Request * data =
      (go2_interfaces__srv__FootRaiseHeight_Request *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!go2_interfaces__srv__FootRaiseHeight_Request__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          go2_interfaces__srv__FootRaiseHeight_Request__fini(&data[i]);
        }
        free(data);
        return false;
      }
  ...
  go2_interfaces__srv__FootRaiseHeight_Response__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    go2_interfaces__srv__FootRaiseHeight_Response * data =
      (go2_interfaces__srv__FootRaiseHeight_Response *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!go2_interfaces__srv__FootRaiseHeight_Response__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          go2_interfaces__srv__FootRaiseHeight_Response__fini(&data[i]);
        }
        free(data);
        return false;
      }
