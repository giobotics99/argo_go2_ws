  ...
  hesai_ros_driver__msg__UdpFrame__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    hesai_ros_driver__msg__UdpFrame * data =
      (hesai_ros_driver__msg__UdpFrame *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!hesai_ros_driver__msg__UdpFrame__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          hesai_ros_driver__msg__UdpFrame__fini(&data[i]);
        }
        free(data);
        return false;
      }
